...
fn build_texture<W: Writer>(mut dest: &mut W, ty: TextureType, dimensions: TextureDimensions) {
    ...
    // writing the `new` function
    {
        ...
        (writeln!(dest, "
                ...
                pub fn new<T>(display: &::Display, data: {param}) -> {name}
                    where T: {data_type}
                {{
        ...
        (writeln!(dest, "let format = PixelValue::get_format(None::< <T as {}>::Pixel>);", data_type)).unwrap();
        match ty {
        ...
        (writeln!(dest, "let client_format = PixelValue::get_format(None::< <T as {}>::Pixel>);", data_type)).unwrap();
    ...
    if dimensions != TextureDimensions::Texture1dArray && dimensions != TextureDimensions::Texture2dArray {
        ...
        (write!(dest, r#"
                ...
                pub fn read<T>(&self) -> T where T: {data_type} {{
                    let data = self.0.read::< <T as {data_type}>::Pixel>(0);
                    {constructor}
                }}
